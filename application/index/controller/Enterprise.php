 ...
 */
class Enterprise extends PCViewLogin {
    ...
    public function main() {
        // tp5写法
        $session = Session::get("curEnterprise");
        $sevice_end_time = $session ['service_end_time'];
        $end_date = strtotime(substr($sevice_end_time, 0, 10));
        $start_date = strtotime("today");
        $betweendays = ($end_date - $start_date) / 60 / 60 / 24;
        $this->betweendays = $betweendays;
        $this->brandname = $session ['e_name']; // 当前企业名字

        $customertable = Db::name('customerinfo');
        ...
        $couponcount = Db::name('customercoupon')->where($couponmap)->count();
        ...
        $serviceinfo = Db::name('e_service_info')->where($servicemap)->order('nav_level asc, sort asc')->select(); // 在视图中找出商家享有的导航
    ...
    public function loginOut() {
        ...
        return ajax(array('errCode' => 0, 'errMsg' => '注销成功!'), 'json');
    }
